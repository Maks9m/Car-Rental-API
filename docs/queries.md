...
### Запит 2: Фінансовий звіт по локаціях (парковках)

**Бізнес-питання:**
Які локації приносять сервісу найбільший дохід і де зосереджена основна активність користувачів? Нам потрібно знати:

Кількість завершених поїздок на кожній локації (популярність місця).

Загальну суму виручки (прибутковість).

Середній чек поїздки, що завершилася на цій адресі.

Це дозволяє бізнесу приймати рішення про розширення парковок у популярних зонах або перегляд тарифів для низькорентабельних локацій.

**SQL-запит (генерований SQLAlchemy):**

```sql
SELECT
    car_location.address AS address,
    COUNT(trip.trip_id) AS total_trips,
    SUM(payment.amount) AS total_revenue,
    AVG(payment.amount) AS avg_trip_cost
FROM car_location
JOIN trip ON car_location.car_location_id = trip.end_location
JOIN payment ON trip.trip_id = payment.trip_id
GROUP BY car_location.address;
```

### Пояснення логіки запиту

Цей запит реалізує фінансову аналітику територіальних одиниць сервісу. Логіка складається з наступних кроків:

Об'єднання даних (JOINs):

Ми починаємо з таблиці car_location (адреси парковок).

Приєднуємо trip через end_location. Це важливо, бо ми рахуємо прибуток саме по точці завершення поїздки (куди пригнали авто).

Приєднуємо payment до кожної поїздки, щоб дістати суму грошей.

Агрегація даних (Functions):

COUNT(trip.trip_id): Рахує фізичну кількість заїздів на локацію.

SUM(payment.amount): Підсумовує всі гроші, отримані за ці поїздки.

AVG(payment.amount): Розраховує "середній чек", що допомагає зрозуміти тип поїздок (короткі поїздки по місту чи тривала оренда).

Групування (GROUP BY):

GROUP BY car_location.address: Ми об'єднуємо всі записи в групи за назвою адреси. Кожен рядок у результаті — це окрема унікальна локація зі своєю статистикою.
